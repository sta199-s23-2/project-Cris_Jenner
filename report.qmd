...
by_education <- counties |>
  group_by(State) |>
  summarize(percent_collegedegree = median(`Education.Bachelor's Degree or Higher`))

maps_dataframe3 <- full_join(statepop, by_education, by = c("abbr" = "State")) 

plot_usmap(data = maps_dataframe3,
           values = "percent_collegedegree",
           labels = TRUE) +
  labs(fill = "Median percent college degree", 
       title = "College Degree Breakdowns By State") +
  scale_fill_viridis_c() +
  theme(legend.position = "bottom") 
...
lm(`Income.Median Houseold Income` ~ `Age.Percent 65 and Older` + 
...
Holding percent of women in each county constant, for a one percent increase in percent of county citizens 65 years or older, we estimate a mean change in median household income of -799.70 dollars.
...
According to the maps, within each state's population, there seems to be a lower percent of females in the northwest and west (\~48% to 50%), while there seems to be a higher percent of females comparatively in the east and southeast regions.

According to the maps, the percent elderly in the US among states seems to be very scattered among regions. Among all states, Montana seems to have the greatest elderly population while Utah and Alaska seem to have the lowest. It seems that states in the northwest seem to have the greatest elderly populations, although the correlation among region seems to be weak.
