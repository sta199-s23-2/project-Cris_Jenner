...
```{r}
#| label: mean_income_95

counties_white95 <- counties |>
  filter(`Ethnicities.White Alone	 not Hispanic or Latino` >= 95)

counties_white95 |>
  summarize(mean_income = mean(`Income.Median Houseold Income`))
```
```{r}
#| label: mean_income_75

counties_nonwhite75 <- counties|>
...
```{r}
#| label: mean_income_55

counties_nonwhite_55 <- counties|>
  filter(`Ethnicities.White Alone	 not Hispanic or Latino` < 55) 

counties_nonwhite_55 |>
  summarize(mean_income_nonwhite = mean(`Income.Median Houseold Income`))
```
